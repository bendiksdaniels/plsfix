...
| Chart/range image export | Yes | Range.getImage() / Chart.getImage() -> base64 PNG; Shape.getImageAsBase64 | all (Mac had an upside-down-image bug issue) | Mechanism for pushing Excel content into PPT as picture. Snippet-sourced; min versions unconfirmed (gap). |
...
| Excel->PPT image + insertion path | Yes | Excel getImage -> PPT shape.fill.setImage(base64); PowerPointApi 1.4 shapes add/move/size/format, 1.8 getImageAsBase64/tables | per host | Practical linking pattern: export base64 client-side, PPT add-in inserts. Snippet-sourced. |
