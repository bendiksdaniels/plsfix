...
- `src/link/` (core, pure): `model` (types + codecs), `crypto` (HKDF/AES-GCM, base64url, tokens), `status` (link state rules, slide fitting), `png` (IHDR size), `workspace` (pairing key + host key store), `relay` (fetch client, typed errors, the batched `fetchLinks` and the relay's item/blob caps) + `wire` (the body shapes the routes promise and the guards that check a 200 against them), `geometry` (A1 rectangles, range intersection), `debounce` (key debouncer with an injectable clock).
- `src/ppt/`: `host.ts` (the only PowerPoint Office.js code: scan by tags, insert, refresh in place - one shape or a whole update-all batch in a single sync - or by reinsertion, break, go to slide, read the active slide), `shapes.ts` (where a shape sits: the shape property list, the PowerPointApi check, walking into groups and addressing a shape through its `groupPath`), `links.ts` (orchestration: list, update, inbox, and the shared `applyBatch` repaint), `fetch.ts` (the fetch half of an update-all: rows grouped per link and key, `POST /api/links/fetch` in chunks of 200, opened here, with the per-row GET as the fallback for what the relay deferred or refused), `revert.ts` ("Revert last update": repaint the ticked rows from the revision below their tag and write that rev back), `main.ts` (pane boot, state and one guarded handler per button), `views.ts` (DOM renderers: link table, inbox list, status and age labels), `actions.ts` (pure: row keys, which rows a button acts on, update detail lines).
...
- `server/`: `lib.rs` router + cache middleware, `store.rs` sqlite (WAL, auth hash, 2 revs, TTL sweep, `get_link_rev` for one named revision), `relay.rs` `/api` routes (bearer, ETag, body limits, `GET /api/links/{id}?rev=<n>` for the revert path), `fetch.rs` (`POST /api/links/fetch`: the changed blobs of a whole deck in one response, bearer-less like the status batch, 200 items and 4 MiB of blobs, the rest named `deferred`), `main.rs` env + bind + hourly sweeper.
