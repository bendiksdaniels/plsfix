...
- `src/pane/links-tab.ts`: the Excel "Links" tab (export, workbook link list, push, go to source, remove, link key); `installLinksTab` is the only thing `main.ts` adds for it. `links-toggles.ts` holds its two workbook-wide tick boxes (auto-push on edit, highlight linked cells): boot restore, the toggle itself and the busy state.
