...
- `src/ppt/`: `host.ts` (the only PowerPoint Office.js code: scan by tags, insert, refresh, break, planned), `links.ts` (orchestration, planned), `views.ts` (DOM renderers, planned), `main.ts` (boot: connection status, requirement check, error reporting, tabs, version footer).
