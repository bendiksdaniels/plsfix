...
- `src/link/` (core, pure): `crypto` (HKDF/AES-GCM, base64url, tokens), `workspace` (pairing key + host key store); planned: `model` (types + codecs), `status` (link state rules, slide fitting), `png` (IHDR size), `relay` (fetch client, typed errors).
