...
//! video/build/plsfix-video.mp4 (draft: 960x540 at 30 fps, plsfix-video-draft.mp4). Also
//! writes the page's layout report.
...
/// The finished file for a full or a draft render.
pub fn output(draft: bool) -> PathBuf {
    build_dir().join(if draft { "plsfix-video-draft.mp4" } else { "plsfix-video.mp4" })
}

pub fn run(draft: bool, workers: u32, track: Option<&Path>) -> Result<()> {
    let started = Instant::now();
    music::run(track)?;
    let port = static_server::serve(video_dir())?;
    let (meta, layout) = {
        let mut probe = CompPage::open(port, Viewport::STAGE)?;
        let layout = probe.cdp.eval("window.__layoutCheck()").context("render: __layoutCheck")?;
        (probe.meta, layout)
    };
    write_file(&build_dir().join("layout.json"), serde_json::to_string_pretty(&layout)?.as_bytes())?;
    ...
    let jobs: Vec<Job> = chunks(tl.frames, workers).into_iter().enumerate().map(|(index, frames)| Job {
        index, frames, fps, vp, port, out: parts_dir.join(format!("part-{index:02}.mp4")),
    }).collect();
