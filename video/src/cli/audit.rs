...
//! copy, and provenance. Exit 1 on any finding.
...
pub fn run() -> Result<()> {
    ...
    println!("audit: green (format, motion, colour, determinism, sound, layout, copy, provenance)");
    Ok(())
}
...
/// with the master it was muxed from (checked in the hero and at the PowerPoint launch).
fn sound(path: &Path) -> Result<Vec<String>> {
    let mut f = Vec::new();
    ...
    for t0 in [5.0, 64.5] {
        let master = video_scan::audio_mono(&music::path(), t0, 1.0)?;
