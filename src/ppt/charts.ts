...
async function drawPlaced(
  ...
  const { box, overlapping, freeSpot, freeSpotSize } = placement;
  remember({ slideId, box, overlapping, freeSpot, freeSpotSize, consume });
  ...
  return { slideId, shapeId, overlapping, freeSpot, freeSpotSize };
}
...
export async function refreshChartGroup(
  ...
  const reason = plan === null ? null : declineReason(plan);
  let silent = false;
  ...
  // the host that stopped answering, or the one that declined the draw.
  return silent ? pictureNote(CHART_HOST_SILENT) : (reason ?? undefined);
}
